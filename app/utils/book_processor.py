...
def detect_extension(filename: str, data: bytes) -> str:
...
def extract_headings_from_docx(path: str) -> Tuple[str, List[Tuple[int, str]]]:
...
def split_into_token_chunks(
    ...
    overlap_pct: float = settings.OVERLAP_PCT,
...
""" async def get_embeddings(texts: List[str]) -> List[List[float]]:
    ...
    resp = openai.Embedding.create(model=settings.EMBEDDING_MODEL, input=texts)
...
def _encode_sync(
    texts: List[str], batch_size: int = 64, normalize: bool = True
) -> List[List[float]]:
    """
