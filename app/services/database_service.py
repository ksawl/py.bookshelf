...
from app.core.database_engine import (
    get_database_engine, 
    get_session_maker,
    get_sync_database_engine,
...
class DatabaseService:
    """Async database service - используется для startup/shutdown"""
    ...
    async def update_job(self, job_id: str, patch: Dict[str, Any]) -> None:
        ...
        filtered_patch = {
            k: v for k, v in patch.items() if k != "job_id" and v is not None
        }
    ...
    async def increment_processed(self, job_id: str, n: int = 1) -> None:
        ...
        async with self.get_session() as session:
            ...
            if job.total_chunks and job.total_chunks > 0:
                progress = max(
                    0, min(100, int((new_processed / job.total_chunks) * 100))
                )
...
class SyncDatabaseService:
    """Synchronous database service - используется в BookshelfService"""
    def __init__(self, database_url: str):
        self.database_url = database_url
        self._session_maker = get_sync_session_maker(database_url)

    def get_session(self) -> Session:
        """Get synchronous database session"""
        return self._session_maker()

    def create_job(self, job_id: str, info: JobInfo) -> None:
        """Create new job record with defaults"""
        job_data = {
            "job_id": job_id,
            "created_at": datetime.utcnow(),
    ...
    def get_job(self, job_id: str) -> Optional[JobInfo]:
        """Get job by ID"""
        with self.get_session() as session:
            result = session.execute(select(Job).where(Job.job_id == job_id))
            job = result.scalar_one_or_none()
            return job.to_dict() if job else None
